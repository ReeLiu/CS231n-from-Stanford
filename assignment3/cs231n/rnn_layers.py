...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  for t in xrange(T):
    # Note you cannot replace the if statement below with simply h[t-1] as it 
    # will result in a mistake in Vanilla RNN: backward, for some reasons I
    # haven't figured out.
    if t == 0:
        h_prev = h0
    else:
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  # Divide the activation vector into four vectors.
  a_i = a[:, :H]
  a_f = a[:, H : 2 * H]
  a_o = a[:, 2 * H : 3 * H]
  a_g = a[:, 3 * H : ]
...
def lstm_backward(dh, cache):
  ...
  x, prev_h, prev_c, Wx, Wh, b, a, a_i, a_f, a_o, a_g, i, f, o, g, next_c, next_h = cache[0]
  D = x.shape[-1]
