...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    # (2) Use a word embedding layer to transform the words in captions_in from 
    ...
    # (4) Use a (temporal) affine transformation to compute scores over the 
    # vocabulary at every timestep using the hidden states, giving an array of 
    # shape (N, T, V).
    scores, cache_scores = temporal_affine_forward(h, W_vocab, b_vocab)
    
    # (5) Compute loss using captions_out, ignoring the points where the output
    # word is <NULL> using the mask above.
    loss, dscores = temporal_softmax_loss(scores, captions_out, mask, verbose=False)
    
    # backward pass
    
    # Initialize keys for grads.
  ...
  def sample(self, features, max_length=30):
    ...
    # The first word that you feed to the RNN
    ...
    for t in xrange(max_length):
        word_embed, cahce_embed = word_embedding_forward(capt, W_embed) 
        if self.cell_type == 'rnn':
            # (2) Make an RNN step using the previous state and the embedded 
            # current word to get the next hidden state.
            next_h, cache_rnn = rnn_step_forward(np.squeeze(word_embed), prev_h, 
                                                Wx, Wh, b)
        elif self.cell_type == 'lstm':
            prev_c = np.zeros_like(h0) # previous cell state
            next_h, next_c, cache_lstm = lstm_step_forward(np.squeeze(word_embed), prev_h,
                                                           prev_c, Wx, Wh, b)
        else:
            raise ValueError('Invalid cell_type "%s"' % self.cell_type)
        ...
        scores, cache_scores = temporal_affine_forward(next_h[:, np.newaxis, :], W_vocab, b_vocab)
        ...
        # Update the hidden state and the current word.
        prev_h = next_h
